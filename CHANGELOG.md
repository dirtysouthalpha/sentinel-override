...
### Fixed
- **The masking feature protected nothing on this fleet.** `shouldScrub()` decided purely on hostname, and every cloud request here is addressed through a LOCAL egress proxy (`127.0.0.1:8901` → z.ai, `:8798` → Anthropic, `:8800` → LongCat). All of it was classified "not third-party egress" and sent unmasked, while Settings said "Cloud providers only (recommended)". A local *host* is not evidence of a local *model*: the policy now also considers the model family and the proxy path, so a cloud model behind a local address is scrubbed and a genuinely self-hosted one (`llama3.2`, `qwen2.5vl:7b` on `:11434`) stays exempt. Found by checking the real fleet endpoints before trusting the unit tests.
- **A reasoning model could burn a full billed call for nothing.** Observed live: `200 25482ms model=glm-4.6 finish=length emptyContent=true`. `generatePlan()` asked for `maxTokens:1200` — a non-thinking budget — so the model spent it all on `reasoning_content`, hit the ceiling, returned empty `content`, and the caller silently degraded to a single-step plan. No error, no non-2xx, no log line. New `background/llm-thinking-budget.js` detects truncation-with-no-usable-content (OpenAI `finish_reason:"length"`, Anthropic `stop_reason:"max_tokens"`, think-block-only content) and retries **once** at 4x, capped at 16000. It deliberately does not fire when a truncated response still carried content — re-billing every verbose-but-usable reply would be the opposite of the point.
- **`resolveProvider(endpoint)` ignored the operator's chosen provider.** It maps everything that isn't Anthropic or Z.AI to `PROVIDERS.openai`, so selecting "Ollama (Local)" still used OpenAI's builders and strict parser; the lenient `parseResponse` and optional-auth headers those definitions carry had never run once. New `resolveProviderForConfig()` with explicit precedence: a non-OpenAI wire format is dictated by the endpoint (the server decides the request shape, not the dropdown), otherwise the operator's selection wins, otherwise endpoint sniffing.
- **Partial provider definitions were returned bare**, dropping everything they don't define — including `systemPromptTweak`, so `buildBody` emitted `{"role":"system"}` with **no content key** and ollama rejected the whole request with `invalid message content type: <nil>`. Local definitions are now layered over OpenAI, and the shared OpenAI/OpenRouter `buildBody` omits the system message rather than emitting a malformed one. **This regression was introduced by the fix above and caught by the live test, not by any unit test** — a unit regression test now covers it.
- **`--forceExit` was hiding a real leak.** `--detectOpenHandles` on the agent-engine suites found exactly one: a test calling `sleep(100000)`, leaving a live 100-second timer. Rewritten with fake timers; the full suite now exits cleanly on its own and `npm test` / `npm run test:quick` no longer pass `--forceExit`, so it can never hide a leak again.

### Added
- **Credential typing policy** (`content/credential-policy.js`). The agent could type into `input[type=password]` unconditionally — the code detected the field, logged "proceeding per IT-tech authorization", and typed, with no setting, no prompt and no way to refuse. Nobody had authorized anything. Now deliberate, logged and consented: default **block** with an explicit human handoff, plus `approve` (asks each time) and `allow` (types, still audited). Unknown or malformed policy values fail safe to block; a missing policy module fails closed rather than typing. The blocked message is written for the *model* — "HUMAN HANDOFF REQUIRED, do not retry, do not try another selector" — so a planner hands off instead of hunting for another route to the same field. Every attempt is written to telemetry and to a forensic run-log event whatever the outcome. Focusing such a field is still allowed (it reveals nothing); its misleading log line is gone.
- **Honest execute_js approval card** (`background/execute-js-risk.js`). Rather than chase intrinsics-freezing in a sandbox that shares a JavaScript realm with the page it guards, the card now names the specific construct that would leave the sandbox — `.constructor.constructor`, `Function`, `eval`, dynamic `import`, computed global access, prototype walks — and states the two facts that were never said out loud: approving **skips the sandbox entirely**, and the sandbox is a guard-rail against model mistakes, not a security boundary. Ordinary DOM extraction is not flagged.
- **Live end-to-end masking verification** in `tests/live/agent-live-smoke.test.js`, run against a real cloud model (glm-4.6 via z.ai) with a planted key, email and phone in the page. Proves all three: the planted values are absent from the real outbound bodies (placeholders and genuine ticket data present), the agent still completed the multi-step task (reached the detail page, `exit=finish`, summary contains TKT-4488), and no `[[TOKEN]]` leaked into the summary or any dispatched action. **Masking did not degrade task success.**
- Settings → Privacy & Data Protection now carries all three controls: masking mode, credential typing, screenshot field masking.
- `tests/upgrade-audit.test.js` (47) and 3 more in `tests/egress-scrub.test.js` (68 total).

### Verified, not changed
- **MFA and sign-in-wall detection already hard-stop.** Both set `agentPaused` and block until the operator clicks Resume, with a desktop notification; the sign-in wall also writes a forensic run-log entry. They are not annotations. Caveats worth knowing: each is suppressed per-URL after one acknowledgement, and both sit inside a `catch` that never rethrows, so a detection fault skips the pause silently.

### Notes
- The thinking-budget retry is thoroughly unit-tested but was **not** observed firing in the final live run — that run's plan call returned `finish=stop`. The truncation is intermittent on glm-4.6.
