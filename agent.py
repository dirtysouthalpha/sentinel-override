...
class Agent:
    ...
    async def call_utility_model(
        ...
        """
        Call utility model with comprehensive error handling and fallback support.

        Args:
            system: System prompt for the utility model
            message: User message for the utility model
            callback: Optional callback for streaming responses
            background: Whether this is a background call (bypasses rate limiting)
            fallback_response: Optional fallback response if all LLM calls fail

        Returns:
            str: The LLM response, or fallback_response if all attempts fail
        """
        model = self.get_utility_model()

        if not model:
            PrintStyle().warning("No utility model configured, using fallback response")
        ...
        try:
            ...
            PrintStyle().warning(f"Utility model pre-call extensions failed: {str(e)}")
        ...
        try:
            ...
            try:
                ...
                PrintStyle().warning(f"Utility model post-call extensions failed: {str(e)}")
                # Still return the response

            return response

        except Exception as e:
            PrintStyle().error(f"Utility model call failed: {str(e)}")

            # Try Neuralis Brain fallback if available
            try:
                from helpers.brain_client import BrainClient
                brain = BrainClient()
                brain_response = brain.query_utility(system, message)
                if brain_response:
                    PrintStyle().success("Neuralis Brain fallback succeeded")
                    return brain_response
                else:
                    PrintStyle().warning("Neuralis Brain fallback returned empty response")
            except ImportError:
                PrintStyle().warning("Neuralis Brain client not available for fallback")
            except Exception as brain_error:
                PrintStyle().error(f"Neuralis Brain fallback failed: {str(brain_error)}")

            # Final fallback to provided response or empty string
            if fallback_response is not None:
                PrintStyle().warning(f"Using fallback response: {fallback_response[:100]}...")
                return fallback_response

            PrintStyle().warning("No fallback response available, returning empty string")
            return ""
