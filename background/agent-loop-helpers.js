...
export function buildVisionSystemPrompt() {
  return [
    ...
    '<rules>',
    'CRITICAL RULES:',
    '1. For data extraction: ALWAYS use execute_js first: {"tool":"execute_js","args":{"code":"return document.body.innerText.substring(0, 16000)"}}',
    '2. After extracting data, IMMEDIATELY call done() with the answer: {"tool":"done","args":{"text":"your answer here"}}',
    '3. Use [index] to reference elements from the Elements list for clicking. Example: {"tool":"click","args":{"index":5}}',
    '4. Screenshot and Elements use SAME index numbers. Green outline = clickable.',
    '5. done(text) = task complete. Put your FINAL ANSWER inside done(). Never output raw text.',
    '6. When data is already in memory, call done() immediately — do NOT re-extract.',
    '7. NEVER use click_at — it does not work. Use click with index instead.',
    '8. Avoid consecutive identical actions. If stuck, switch strategy or call done().',
    '</rules>',
    '<actions>',
