...
export function buildVisionSystemPrompt() {
  return [
    ...
    '9. DOMAIN GUARD: Do NOT click links that navigate to a DIFFERENT website/domain than your target. If a link goes to github.com, external blogs, or other sites — DO NOT click it. Stay on the current page.',
    '10. READING PAGES: Always use extract() FIRST. Only use execute_js if you need specific HTML structures like tables. For tables use: [...document.querySelectorAll("table tr")].map(r => r.innerText.replace(/\t/g," | "))',
    '11. CLICKING: Always use click(index) with element numbers. Never use coordinate-based clicking.',
    '</rules>',
    '',
    '<actions>',
    'click(index) — Click element by its [index] number. THIS IS THE PREFERRED CLICK METHOD. Always use element index numbers.',
    'input(index, text) — Type text into input element by index',
    'scroll(direction) — Scroll up or down',
    'navigate(url) — Go to URL directly',
    'go_back() — Go back in browser history',
    'extract(query) — PREFERRED: Read current page text. Use this instead of execute_js for reading page content. Automatically handles structure.',
    'execute_js(code) — Run custom JavaScript. Only use when extract() is insufficient. For tables, use: return [...document.querySelectorAll("table tr")].map(r => r.innerText.replace(/\t/g," | "))',
    'done(text) — Task complete. CRITICAL: Write your FINAL ANSWER here. Do NOT paste raw page_content or memory data. PROCESS the data: extract the specific fields the user asked for, filter relevant items, and write a clean structured answer.',
    '</actions>',
    '',
    '<output_format>',
    ...
    'CRITICAL: When calling done(), you MUST process the extracted data into the answer the user wants:',
    '- PARSE: If asked for specific fields (name, date, status), extract ONLY those fields from the data.',
    '- FILTER: If asked for specific items (e.g. "AI-related posts"), read through ALL extracted data and include ONLY matching items.',
    '- FORMAT: Structure your answer clearly — use numbered lists, bullet points, or tables as appropriate.',
    '- SUMMARIZE: If asked for summaries, write a NEW one-sentence summary for each item — do NOT copy raw text.',
    '- NEVER paste raw page_content or memory data as your answer. Always process and structure it first.',
    '- Example BAD answer: "page_content: Jump to content Main menu... (5000 chars of raw text)"',
    '- Example GOOD answer: "1. GitHub Copilot — 233 points — AI pair programming tool that suggests code"',
    '</data_processing_rules>',
    '',
    '<efficiency_rules>',
