  ...
  return new Promise((resolve, reject) => {
    ...
    chrome.tabs.sendMessage(tabId, message, (response) => {
      ...
      if (chrome.runtime.lastError) {
        reject(new Error(chrome.runtime.lastError && chrome.runtime.lastError.message || 'Content script message failed'));
        return;
      }
      if (!response) {
...
export function sendRuntimeMessage(message, timeoutMs = 10000) {
  return new Promise((resolve, reject) => {
    ...
    chrome.runtime.sendMessage(message, (response) => {
      clearTimeout(timeout);
      if (chrome.runtime.lastError) {
        reject(new Error(chrome.runtime.lastError && chrome.runtime.lastError.message || 'Runtime message failed'));
        return;
      }
      resolve(response);
    });
