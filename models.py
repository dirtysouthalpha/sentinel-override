...
# === A0 overload fallback (DirtySouthAlpha) ===
# When the primary model exhausts retries (z.ai down / overloaded / quota),
# unified_call swaps to this backup provider for one more attempt.
# homeserver (SENTINEL-CORE) Ollama over Tailscale; reachable from host-networked A0.
A0_FALLBACK = {
    "enabled": True,
    "model": "openai/glm-5.2",   # z.ai coding (OpenAI-compat); cloud backup to LongCat primary
    "api_base": "https://api.z.ai/api/coding/paas/v4",   # z.ai coding endpoint
    "api_key": "d120d04790f54ce788e8a1bfbbb19f1f.TAssEoF2oIhxIppE",                         # z.ai coding key (rotated 2026-06-25)
}
...
class LiteLLMChatWrapper(SimpleChatModel):
    ...
    async def unified_call(
        ...
        **kwargs: Any,
        ...
        max_retries: int = int(call_kwargs.pop("a0_retry_attempts", 5))
        initial_retry_delay_s: float = float(call_kwargs.pop("a0_retry_delay_seconds", 2.0))
        max_retry_delay_s: float = float(call_kwargs.pop("a0_max_retry_delay_seconds", 60.0))
        stream = reasoning_callback is not None or response_callback is not None or tokens_callback is not None

        # results
        result = ChatGenerationResult()

        # Failover support: if the primary model exhausts retries, retry once on
        # the backup provider (A0_FALLBACK). active_* let us swap model+endpoint mid-call.
        active_model = self.model_name
        active_kwargs = dict(call_kwargs)
        fallback_used = False
        ...
        while True:
            got_any_chunk = False
            try:
                # call model (active_model/active_kwargs enable failover to A0_FALLBACK)
                ...
                if is_rate_limit:
                    PrintStyle().warning(f"Rate limit detected on attempt {attempt + 1}/{max_retries + 1}")
                else:
                    PrintStyle().error(f"Model call attempt {attempt + 1}/{max_retries + 1} failed: {error_type}: {error_msg}")

                # Retry conditions: rate limits OR transient errors (if no chunks received yet)
                should_retry = (
                    is_rate_limit or
                    (not got_any_chunk and _is_transient_litellm_error(e))
                ...
                if should_retry:
                    ...
                    if is_rate_limit:
                        # Longer, more aggressive backoff for rate limits
                        delay = min(
                            initial_retry_delay_s * (3 ** attempt),  # Exponential base 3 for rate limits
                            max_retry_delay_s
                        )
                        PrintStyle().print(f"Rate limit retry {attempt}/{max_retries}, waiting {delay:.1f}s...")
                    else:
                        # Standard exponential backoff for transient errors
                        delay = min(
                            initial_retry_delay_s * (2 ** attempt),  # Exponential base 2 for transient errors
                            max_retry_delay_s
                        )
                ...
                # FALLBACK: primary exhausted its retries -> try the backup provider once.
                if not fallback_used and A0_FALLBACK.get("enabled"):
                    fallback_used = True
                    active_model = A0_FALLBACK["model"]
                    active_kwargs = {
                        **call_kwargs,
                        "api_base": A0_FALLBACK["api_base"],
                        "api_key": A0_FALLBACK["api_key"],
                    }
                    attempt = 0  # give the backup its own retry budget
                    PrintStyle().warning(
                        f"Primary model exhausted after retries; falling back to {active_model} @ {A0_FALLBACK['api_base']}"
                    )
                    continue

                # If we get here, retry exhausted or non-retryable error
                if is_rate_limit:
                    raise Exception(f"Rate limit error after {attempt + 1} attempts: {error_msg}") from e
                else:
                    raise type(e)(f"Model call failed after {attempt + 1} attempts: {error_msg}") from e
