...
class RecallMemories(Extension):
    ...
    async def search_memories(self, log_item: log.LogItem, loop_data: LoopData, **kwargs):
        ...
        if set["memory_recall_query_prep"]:
            try:
                # call util llm to generate search query from the conversation with fallback
                query = await self.agent.call_utility_model(
                    system=system,
                    message=message,
                    # callback=log_callback,
                    fallback_response=user_instruction  # Fallback to original user message
                )
                query = query.strip()
                ...
                # Use fallback to original user instruction instead of failing completely
                query = user_instruction
                log_item.update(query=query, progress="Using fallback query due to error")

            # no query, no search
            if not query:
                log_item.update(
                    heading="No memory query available, using original message",
                )
                query = user_instruction  # Final fallback to original message
        ...
        if set["memory_recall_post_filter"]:
            ...
            # call AI to validate the memories with fallback
            try:
                filter = await self.agent.call_utility_model(
                    ...
                    fallback_response=json.dumps(list(range(len(mems_list))))  # Fallback: keep all memories
                )
                filter_inds = dirty_json.try_parse(filter)

                # If filter fails, use fallback to keep all memories
                if not filter_inds or not isinstance(filter_inds, list):
                    log_item.update(progress="Filter parsing failed, keeping all memories")
                    filter_inds = list(range(len(mems_list)))
