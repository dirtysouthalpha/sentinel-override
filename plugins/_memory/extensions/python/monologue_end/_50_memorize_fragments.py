...
class MemorizeMemories(Extension):
    ...
    async def memorize(self, loop_data: LoopData, log_item: LogItem, **kwargs):
        ...
        try:
            ...
            # call util llm to find info in history with fallback
            memories_json = await self.agent.call_utility_model(
                system=system,
                message=msgs_text,
                # callback=log_callback,
                background=True,
                fallback_response="[]"  # Fallback to empty list
            )
            ...
            if not memories_json or not isinstance(memories_json, str):
                log_item.update(heading="No response from utility model, skipping memorization.")
                return

            # Strip any whitespace that might cause issues
            memories_json = memories_json.strip()

            if not memories_json or memories_json == "[]":
                log_item.update(heading="Empty response from utility model, skipping memorization.")
                return
