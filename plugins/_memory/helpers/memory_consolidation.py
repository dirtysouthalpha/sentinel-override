...
class MemoryConsolidator:
    ...
    async def _extract_search_keywords(
        ...
        try:
            ...
            # Call utility LLM to extract search queries with fallback response
            keywords_response = await self.agent.call_utility_model(
                system=system_prompt,
                message=message_prompt,
                background=True,
                fallback_response=json.dumps([])  # Fallback to empty list
            )

            if not keywords_response:
                if log_item:
                    log_item.update(progress="Keyword extraction returned empty, using fallback")
                return self._get_fallback_keywords(new_memory)

            # Parse the response - expect JSON array of strings
            try:
                keywords_json = DirtyJson.parse_string(keywords_response.strip())
            except Exception as parse_error:
                PrintStyle().warning(f"Failed to parse keywords response: {str(parse_error)}")
                if log_item:
                    log_item.update(progress="Keyword parsing failed, using fallback")
                return self._get_fallback_keywords(new_memory)

            if isinstance(keywords_json, list):
                valid_keywords = [str(k) for k in keywords_json if k]
                if valid_keywords:
                    return valid_keywords
                else:
                    return self._get_fallback_keywords(new_memory)
            elif isinstance(keywords_json, str):
                return [keywords_json] if keywords_json.strip() else self._get_fallback_keywords(new_memory)
            else:
                return self._get_fallback_keywords(new_memory)
            ...
            if log_item:
                log_item.update(progress=f"Keyword extraction error: {str(e)}, using fallback")
            return self._get_fallback_keywords(new_memory)

    def _get_fallback_keywords(self, new_memory: str) -> List[str]:
        """Generate fallback search keywords when LLM extraction fails."""
    ...
    async def _analyze_memory_consolidation(
        ...
        """Use LLM to analyze memory consolidation options with comprehensive error handling."""
        ...
        try:
            ...
            # Call utility model with fallback
            analysis_response = await self.agent.call_utility_model(
                system=system_prompt,
                message=message_prompt,
                callback=None,
                background=True,
                fallback_response='{"action": "skip", "reasoning": "LLM call failed, using safe fallback"}'
            )

            if not analysis_response:
                if log_item:
                    log_item.update(progress="LLM consolidation returned empty response, using safe fallback")
            ...
            try:
                ...
                PrintStyle().warning(f"Failed to parse consolidation response: {str(parse_error)}")
                if log_item:
                    log_item.update(progress=f"Parse error: {str(parse_error)}, using safe fallback")
                return ConsolidationResult(
                    action=ConsolidationAction.SKIP,
            ...
            if not isinstance(result_json, dict):
                PrintStyle().warning(f"LLM response is not a valid JSON object, got: {type(result_json)}")
                if log_item:
                    log_item.update(progress="Invalid JSON structure, using safe fallback")
                return ConsolidationResult(
                    action=ConsolidationAction.SKIP,
                    reasoning="Invalid JSON structure from LLM"
                )

            # Parse consolidation result with validation
            action_str = result_json.get('action', 'skip')
            try:
                action = ConsolidationAction(action_str.lower())
            except ValueError:
                PrintStyle().warning(f"Unknown consolidation action: {action_str}, using SKIP")
                action = ConsolidationAction.SKIP
            ...
            # Validate memory IDs exist before trying to remove/update them
            memories_to_remove = []
            if result_json.get('memories_to_remove'):
                valid_ids = [doc.metadata.get('id') for doc in context.similar_memories if doc.metadata.get('id')]
                memories_to_remove = [mid for mid in result_json.get('memories_to_remove', []) if mid in valid_ids]

            return ConsolidationResult(
                action=action,
                memories_to_remove=memories_to_remove,
            ...
            if log_item:
                log_item.update(progress=f"Analysis error: {str(e)}, using safe fallback")
            # Fallback: skip consolidation with original memory
            return ConsolidationResult(
                action=ConsolidationAction.SKIP,
                new_memory_content=context.new_memory,  # Use original memory
