...
function addActionCard(payload) {
  ...
  if (pendingStepLogs[payload.stepNumber]) {
    pendingStepLogs[payload.stepNumber].forEach(text => appendLogLine(payload.stepNumber, text));
    delete pendingStepLogs[payload.stepNumber];
  }
...
function appendLogLine(stepNumber, text) {
  ...
  if (!logArea) {
    ...
    if (!pendingStepLogs[stepNumber]) pendingStepLogs[stepNumber] = [];
    pendingStepLogs[stepNumber].push(text);
    return;
  }
...
const COMMANDS = [
  ...
  { name: 'About', desc: 'About Sentinel Override', action: 'about', icon: 'ℹ️' },
];
