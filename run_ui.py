...
def prepare_web_runtime() -> tuple[UiServerRuntime, str, int]:
    ...
    # Register SystemHealthHandler manually for /system_health namespace
    from python.websocket_handlers.system_health_handler import SystemHealthHandler
    import threading
    manager = server_runtime.ws_manager
    handler = SystemHealthHandler(server_runtime.socketio_server, threading.RLock(), manager=manager, namespace='/system_health')
    manager.register_handlers({'/system_health': [handler]})
