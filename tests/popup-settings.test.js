...
describe('settings.js — switchProviderCard', () => {
  ...
  test('sets anthropic defaults for anthropic provider', () => {
    ...
    expect(ep._attrs.placeholder).toContain('anthropic.com');
    expect(model._attrs.placeholder).toContain('claude');
  });

  test('sets openai defaults for openai provider', () => {
    sandbox.switchProviderCard('openai');
    const ep = sandbox._elCache['set-provider-endpoint'];
    const model = sandbox._elCache['set-provider-model'];
    expect(ep._attrs.placeholder).toContain('openai.com');
