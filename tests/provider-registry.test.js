...
describe('getCatalogProvider', () => {
  ...
  test('catalog has expected entries', () => {
    // Catalog grows as new providers are added — check for required entries
    const ids = PROVIDER_CATALOG.map(p => p.id);
    const required = ['openai', 'anthropic', 'google', 'deepseek', 'openrouter', 'groq',
      'mistral', 'together', 'fireworks', 'ollama', 'lmstudio', 'zai', 'custom',
      'cerebras', 'perplexity', 'venice', 'cohere', 'ai21', 'xai',
      'koboldcpp', 'tabbyapi', 'tgi', 'jan', 'chutes', 'samba', 'cloudflare', 'replicate'];
