...
const model = {
  ...
  async loadSettings() {
    try {
      ...
      if (settings) {
        ...
        this.autoSpeak = settings.auto_speak ?? this.autoSpeak;
      }
    } catch (error) {
  ...
  async speakStream(id, text, finished = false) {
    // if already running the same stream with identical text and state, do nothing
    if (
      this.ttsStream &&
    ...
    // Skip if this is an older/smaller text than what we've already processed
    if (this.ttsStream && this.ttsStream.id === id && text.length <= this.ttsStream.lastSpokenPosition) {
      return;
    }
    ...
    if (!this.ttsStream || this.ttsStream.id !== id) {
      ...
      this.ttsStream = {
        ...
        running: false,
        lastChunkIndex: -1,
        lastSpokenPosition: 0,  // Track position in original text to prevent repetition
        spokenChunks: new Set(), // Track exact chunk texts that have been spoken
        stopped: false,
        chunks: [],
      };
    } else {
    ...
    while (true) {
      ...
      // Check if this chunk is ready to speak
      const isLastChunk = nextIndex == this.ttsStream.chunks.length - 1;
      const isFinished = this.ttsStream.finished;

      // For the last chunk in an unfinished stream, check if it's a complete sentence
      if (isLastChunk && !isFinished) {
        const chunk = this.ttsStream.chunks[nextIndex];
        const endsWithSentence = /[.!?]$/.test(chunk.trim());

        // Only wait if the chunk doesn't end with sentence punctuation
        if (!endsWithSentence) {
          // wait a bit for more content or finish signal
          await new Promise((resolve) => setTimeout(resolve, 50));
          continue;
        }
        // If it ends with sentence punctuation, speak it immediately
      }
      ...
      // Skip if this exact chunk has already been spoken (prevents repetition)
      if (this.ttsStream.spokenChunks.has(chunk)) {
        this.ttsStream.lastChunkIndex = nextIndex;
        continue;
      }

      // Mark this chunk as spoken
      this.ttsStream.spokenChunks.add(chunk);
      spoken.push(chunk);

      // Update the spoken position based on chunk content
      // Find where this chunk appears in the original text and track position
      const chunkPosition = this.ttsStream.text.indexOf(chunk, this.ttsStream.lastSpokenPosition);
      if (chunkPosition >= 0) {
        this.ttsStream.lastSpokenPosition = chunkPosition + chunk.length;
      }
  ...
  chunkText(text, { maxChunkLength = 135, lineSeparator = "..." } = {}) {
    ...
    const MIN_CHUNK_LENGTH = 10; // minimum length for a chunk before merging
